...
`voleystatslive` es el **visor web público** del ecosistema Clipeo (apps iOS/Android; nombre de marca hasta 2026: Voley Stats — el repo y el proyecto Firebase compartido conservan el nombre antiguo, ver más abajo). No tiene backend propio: **lee** en tiempo real (o desde caché) el mismo proyecto Firebase/Firestore en el que las apps escriben, y lo renderiza. Coupling único con `../VoleyStatsApp/` y `../VoleyStats-Android/`: Firestore. El contrato de campos es autoritativo en `../VoleyStatsApp/CLAUDE.md` ("Companion web app" / "Live sets scoreboard") — este repo solo lee, nunca escribe.
...
- `/:code([A-Za-z0-9]{15,})` → enlace corto (`clipeostats.com/<código>`); redirige a `stats`. El patrón (15+ alfanuméricos) evita capturar otras rutas — los códigos son IDs de Firestore.
